...
/// PKCE), supply only `provider` and `idToken` (the authorization code from
/// the callback URL) — the provider manages the PKCE verifier and state
/// internally when you use `authorizationRequest(state:)`.
///
/// Pass `state` when the authorization server echoes a state parameter back
/// in the callback URL. `OAuth2Provider.signIn(with:)` validates it against
/// the value generated by `authorizationRequest(state:)` before making any
/// network call.
public struct OAuthCredential: AuthCredential {
    ...
    /// The PKCE `code_verifier`. Only needed when constructing a credential
    /// manually without calling `authorizationRequest(state:)` first.
    public let nonce: String?
    /// The `state` parameter returned by the authorization server in the
    /// callback URL. Pass `nil` when the server does not echo state back.
